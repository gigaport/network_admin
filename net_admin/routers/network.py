...
from utils.cisco_multicast import GetCiscoMulticastInfo
from utils.arista_multicast import GetAristaMulticastInfo

# 스레드풀 생성
executor = ThreadPoolExecutor(max_workers=60)

CISCO_TS_DEVICES = load('../common/ts_member_mpr.yaml')
CISCO_PR_DEVICES = load('../common/pr_member_mpr.yaml')

ARISTA_PR_DEVICE_PATH = '../common/pr_information_mkd.json'
...
@router.get("/collect/multicast/cisco/{target}")
async def CollectCiscoMulticast(target: str):
    ...
    tasks = [
        loop.run_in_executor(executor, GetCiscoMulticastInfo, device_info, device_name)
