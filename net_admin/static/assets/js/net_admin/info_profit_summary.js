        ...
        profitTable = $('#profitTable').DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function(data) {
                        if (!data && data !== 0) return '-';
                        return '<span style="color: ' + T('#1e293b','#e2e8f0') + '; font-weight: 700;">' + Number(data).toLocaleString() + '원</span>';
                    }
                },
                {
                    targets: 8, // 매입회선수
            ...
            drawCallback: function(settings) {
                ...
                var tMkdCount = 0, tRevenue = 0, tPurchaseCount = 0, tPurchase = 0, tProfit = 0;
                data.each(function(row) {
                    tMkdCount += Number(row.mkd_count) || 0;
                    tRevenue += Number(row.revenue_total) || 0;
                    tPurchaseCount += Number(row.purchase_count) || 0;
                ...
                var grandRow = '<tr class="grand-total-row" style="background: #1e293b !important; pointer-events: none;">' +
                    ...
                    '<td class="text-center py-2 align-middle" style="font-weight: 700; color: #cbd5e1;">' + tMkdCount.toLocaleString() + '</td>' +
                    '<td class="text-end py-2 align-middle" style="font-weight: 800; color: #f1f5f9;">' + tRevenue.toLocaleString() + '원</td>' +
                    '<td class="text-center py-2 align-middle" style="font-weight: 700; color: #cbd5e1;">' + tPurchaseCount.toLocaleString() + '</td>' +
