...
def get_info_revenue_summary(request):
    """정보이용사 매출내역 데이터 조회 (FastAPI 호출)"""
    try:
        api_url = f"{FASTAPI_BASE_URL}/api/v1/network/info_revenue_summary"
        logger.info(f"[CALL_API] ==> {api_url}")
        response = requests.get(api_url)
        if response.status_code == 200:
            return JsonResponse(response.json())
...
def get_info_revenue_monthly(request):
    ...
    try:
        year_month = request.GET.get('year_month', '')
        api_url = f"{FASTAPI_BASE_URL}/api/v1/network/info_revenue_monthly?year_month={year_month}"
        logger.info(f"[CALL_API] ==> {api_url}")
        response = requests.get(api_url)
        if response.status_code == 200:
            return JsonResponse(response.json())
